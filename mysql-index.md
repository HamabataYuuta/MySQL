...
```
- 複合インデックスは、定義されたカラム名1を元にして検索を行うことができる。カラム名1で検索をした後にカラム名2で検索をかけることでテーブル内を検索するよりも早く検索できる。
### ユニークインデックスについて　　
...
- 2行目のカラム名がfilm_idのSeq_in_indexを確認すると、インデックスのフィールド番号が2となっている。番号が2というのは、複合インデックスが定義されたときに2番目のカラムとしてインデックス作成されたということである。これにより、PRIMARYキーのactor_idとfilm_idは複合インデックスであるとわかる。インデックスによる検索はactor_idを元にしないとfilm_idが分からないということなので(複合インデックスの作成参照)、film_idを優先するとactor_idはインデックスからは分からない。このため、film_idを優先したクエリはフルテーブルスキャンになっている。

### 処理が遅いクエリ  
- Cardinalityが高いカラムに対してインデックスを貼り付けてないときに、そのカラムを集計する(特に、order byで並び替えをする)と、処理は遅くなる。  
- 複合インデックスを使用しているが、絞込みにカラム名を片方しか入れていない場合や、検索条件が複合インデックスのカラムの登録順に沿っていない場合、インデックスがフルスキャンされるため、処理は遅くなる。 
